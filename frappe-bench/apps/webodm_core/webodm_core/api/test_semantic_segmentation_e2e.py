"""End-to-end: semantic segmentation over a real orthophoto via the live service.

Requires the geospatial image with the semantic-segmentation op (ONNX Runtime +
the SegFormer land-cover model) running as ``geospatial`` on the compose backend
network, and a small real image fixture (``/tmp/opencode/bus_ortho.tif``).
"""
...
MODEL = "segformer-satellite-landcover.onnx"
LABELS = "satellite-landcover.txt"
...
class TestSemanticSegmentationE2E(FrappeTestCase):
    ...
    def setUpClass(cls):
        ...
        assert PLUGIN in result["synced"], "geospatial catalog not reachable"

        cls.owner = _user("seg_owner@example.com")
        cls.org = _org(ORG)
    ...
    def _cleanup(cls):
        for name in frappe.get_all("WebODM Plugin Run",
                                   filters={"plugin": PLUGIN}, pluck="name"):
    ...
    def test_segmentation_end_to_end(self):
        self._enable(settings={"model": MODEL, "labels": LABELS, "min_segment_area": 64})

        self._as(self.owner)
        with patch.object(frappe, "enqueue", lambda *a, **k: None):
            run_name = plugins_api.run_plugin(plugin=PLUGIN, task=self.task_name)["run"]
        frappe.set_user("Administrator")
        frappe.local.webodm_org_cache = {}
        ...
        self.assertTrue(metadata["areas"], "expected per-class areas")
        self.assertEqual(metadata["area_unit"], "m2")
        ...
        for key in ("class", "class_id", "area", "confidence"):
    ...
    def test_missing_model_rejected_before_run(self):
        ...
        before = frappe.db.count("WebODM Plugin Run", {"plugin": PLUGIN})

        self._as(self.owner)
        with patch.object(frappe, "enqueue", lambda *a, **k: None):
            with self.assertRaises(frappe.ValidationError):
                plugins_api.run_plugin(plugin=PLUGIN, task=self.task_name)

        self.assertEqual(frappe.db.count("WebODM Plugin Run", {"plugin": PLUGIN}), before)
    ...
    def test_platform_disabled_rejected(self):
        self._enable(settings={"model": MODEL, "labels": LABELS})
        original = frappe.db.get_value("WebODM Plugin", PLUGIN, "platform_enabled")
        try:
            frappe.db.set_value("WebODM Plugin", PLUGIN, "platform_enabled", 0)
            self._as(self.owner)
            with patch.object(frappe, "enqueue", lambda *a, **k: None):
                with self.assertRaises(frappe.PermissionError):
                    plugins_api.run_plugin(plugin=PLUGIN, task=self.task_name)
        finally:
            frappe.db.set_value("WebODM Plugin", PLUGIN, "platform_enabled", original)
            frappe.set_user("Administrator")
            frappe.local.webodm_org_cache = {}
