...
| Examples | Hillshade, Contours, Object detection, Segmentation | Anything you write in Python |
...
| Key | Meaning |
|---|---|
| `inputs` | One absolute path per input declared in the manifest, keyed by the input's `name`. These are private copies; you may read them freely. |
...
The sandbox interpreter has **numpy**, **rasterio** (GDAL), **shapely** and
the Python standard library. It cannot install packages and has no network
access, so pure-Python helpers must ship inside your package.
...
| Package size | 50 MB zipped, 200 MB unpacked, 500 files |
...
| `inputs` | yes | List of `{name, datasets}`. `datasets` are task fields, first available wins: `orthophoto`, `dsm`, `dtm`, `point_cloud`, `model`. The run is refused up front if the task has none of them. |
...
symlinks, at most 50 MB zipped / 200 MB unpacked / 500 files. Tests and
